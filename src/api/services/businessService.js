...
export const businessService = {
    ...
    try {
      ...
      return {
        ...
        error:
          error.response?.data?.message ||
          error.response?.data?.detail ||
          "Failed to add car dealership business.",
      };
    }
  },
  ...
  updateRedirectUrl: async (businessId, redirectUrl) => {
    try {
      const response = await api.patch(`/business/${businessId}/redirect-url`, {
      ...
      return {
        success: false,
        error:
          error.response?.data?.message || "Failed to update redirect URL.",
        details: error.response?.data,
      };
    }
  },
  ...
  getPublicBusinessById: async (businessId) => {
    try {
      let url = `/business/public/${businessId}`;
      const params = {};

      // If businessId is missing, is a placeholder, or looks like a URL/domain
      if (
        !businessId ||
  ...
  businessLink: async (businessId) => {
    try {
      const response = await api.post(
        `/v1/business/${businessId}/google-calendar/send-connect-email`,
      );
      return {
        success: true,
        data: response.data,
  ...
  instanceLink: async (instanceId) => {
    try {
      const response = await api.post(
        `/v1/instance/${instanceId}/google-calendar/send-connect-email`,
      );
      return {
        success: true,
        data: response.data,
  ...
  createService: async (businessId, serviceData) => {
    try {
      const response = await api.post(
        `/v1/business/${businessId}/services`,
        serviceData,
      );
      return {
        success: true,
  ...
  getStaffServices: async (businessId, instanceId) => {
    try {
      const response = await api.get(
        `/v1/business/${businessId}/instances/${instanceId}/pricing`,
      );
      return {
        success: true,
        data: response.data,
      ...
      return {
        success: false,
        error:
          error.response?.data?.message || "Failed to fetch staff services.",
      };
    }
  },
  ...
  updateMasterService: async (businessId, serviceId, serviceData) => {
    try {
      const response = await api.put(
        `/v1/business/${businessId}/services/${serviceId}`,
        serviceData,
      );
      return {
        success: true,
  ...
  verifyCheckoutSession: async (sessionId) => {
    try {
      const response = await api.get(
        `/stripe/verify-checkout-session/${sessionId}`,
      );
      return {
        success: true,
        data: response.data,
  ...
  createReservation: async (businessId, reservationData) => {
    try {
      const response = await api.post(
        `/v1/business/${businessId}/reservations`,
        reservationData,
      );
      return {
        success: true,
