...
import { CheckCircle2, Calendar, Clock, ArrowRight, Home, Loader2, AlertCircle } from 'lucide-react';
...
const PaymentSuccess = () => {
  ...
  useEffect(() => {
    const verifySession = async () => {
      ...
      try {
        setVerifying(true);
        const response = await businessService.verifyCheckoutSession(sessionId);
        if (response.success) {
          // Clear booking data on successful verification
          localStorage.removeItem(`booking_data_${businessId}`);
        } else {
      ...
      <div className="max-w-md w-full bg-white rounded-[2.5rem] border border-[var(--border-primary)] shadow-2xl overflow-hidden animate-in zoom-in duration-500">
        {verifying ? (
          <div className="p-12 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Please wait while we confirm your booking...</p>
          </div>
        ) : verificationError ? (
          <div className="p-12 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <h2 className="text-xl font-black uppercase tracking-tighter text-red-600">Verification Pending</h2>
            <p className="text-xs font-bold text-gray-500 leading-relaxed">
              {verificationError || "We couldn't confirm your payment status immediately. Please check your email for confirmation."}
            </p>
            <button 
              onClick={() => window.location.reload()}
              className="px-6 py-3 bg-black text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:opacity-90 transition-all"
            >
              Retry Verification
            </button>
          </div>
